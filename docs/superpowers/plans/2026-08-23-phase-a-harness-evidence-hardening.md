...
> **Completion status:** Tasks 1-4 are complete. Their original TDD steps remain as historical
> evidence; Rounds 2-8 record the implemented contracts and their proportionate verification.
...
- [x] Write a regression that mutates the observed protocol/task/environment receipts consistently and proves the independently held expectation still returns `STALE`.
- [x] Run the focused regression and verify RED because `EvidenceReceipt` and `TraceExpectation` do not exist.
- [x] Implement the receipt and expectation contracts, remove `expected_binding` from the trace, and update freshness hashing.
- [x] Add upper bounds for artifact sizes, transformation/tool/environment sequences, token metadata, and schema-version integers.
- [x] Run trace and strict-parsing tests to GREEN.
...
- [x] Write regressions proving a bare verifier boolean is rejected and missing diagnostic families cannot yield `VALID`.
- [x] Run those regressions and verify RED against the old signature and empty-finding behavior.
- [x] Add exact verifier/checker identity/result receipts and require all ten diagnostic families once in enum order.
- [x] Recompute assessment IDs, reasons, evidence IDs, and hashes from the new inputs.
- [x] Run reward and leakage tests to GREEN.
...
- [x] Write a receipt-spoof regression and an 8-by-8 analysis regression.
- [x] Run both and verify RED because cells still trust booleans and analysis rejects 1,232 comparisons.
- [x] Resolve exact receipts against validated snapshots and add typed mismatch/stale/invalid confounds.
- [x] Set the comparison collection bound to the proven 24,512 worst case while retaining the 256-cell grid bound.
- [x] Run matrix and campaign compatibility tests to GREEN.
